...
-- RELACIoN: promocion aplicada a serviciospromotions
...
-- TRIGGERS: 
-- Trigger-1. Actualiza la columna updated_at en users
-- Cada vez que se hace un UPDATE sobre users,
-- se guarda automáticamente la fecha/hora actual
-- ===========================================
...
-- Trigger-2. Calcula appointment_end en appointments
-- Cuando se inserta una cita, suma la duración
-- del servicio a appointment_start
-- ===========================================

DELIMITER $$
CREATE TRIGGER trg_set_appointment_end
...
BEGIN
    ...
    -- Calcular appointment_end
    SET NEW.appointment_end = DATE_ADD(NEW.appointment_start, INTERVAL dur MINUTE);
END$$
DELIMITER ;


-- Trigger-3. Evita el cruce de horarios en citas
-- Si el trabajador ya tiene otra cita en el mismo rango,
-- no permite insertar la nueva
-- ===========================================
...
-- Trigger-4.  Ajusta pagos si la cita está cubierta por un plan
-- Si appointments.user_plan_id no es NULL:
--  - amount = 0
--  - method = 'SUBSCRIPTION'
--  - covered_by_subscription = 1
-- ===========================================

DELIMITER $$
CREATE TRIGGER trg_payment_subscription
BEFORE INSERT ON payments
FOR EACH ROW
BEGIN
    DECLARE plan_id BIGINT;

    -- Obtener si la cita está vinculada a un plan
    SELECT user_plan_id INTO plan_id
    FROM appointments
    WHERE id = NEW.appointment_id;

    -- Si la cita está cubierta por un plan
    IF plan_id IS NOT NULL THEN
        SET NEW.amount = 0;
        SET NEW.method = 'SUBSCRIPTION';
        SET NEW.covered_by_subscription = 1;
    END IF;
END$$
DELIMITER ;


-- TABLA DE AUDITORÍA DE USERS
-- ===========================================
-- Esta tabla guarda un historial de cada vez que
-- se inserta o actualiza un usuario en la tabla "users".
-- Contiene:
--   - El ID del usuario afectado
--   - El tipo de acción ('INSERT' o 'UPDATE')
--   - La fecha/hora de la acción
--   - Los datos del usuario en ese momento
-- ===========================================

CREATE TABLE IF NOT EXISTS users_audit (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    user_id BIGINT NOT NULL,                   -- referencia al usuario afectado
    action VARCHAR(20) NOT NULL,               -- 'INSERT' o 'UPDATE'
...
-- Trigger-5.1 Registra auditoría al insertar usuario
-- ===========================================
-- Cada vez que se inserta un nuevo usuario en "users":
--   - Este trigger se dispara automáticamente
--   - Inserta un registro en "users_audit" con:
--       -> user_id
--       -> acción 'INSERT'
--       -> datos actuales del usuario insertado
-- ===========================================
...
FOR EACH ROW
BEGIN
    INSERT INTO users_audit (user_id, action, username, email, phone, dni, role_id)
    VALUES (NEW.id, 'INSERT', NEW.username, NEW.email, NEW.phone, NEW.dni, NEW.role_id);
END$$
DELIMITER ;


-- Trigger-5.2 Registra auditoría al actualizar usuario
-- ===========================================
-- Cada vez que se actualiza un usuario en "users":
--   - Este trigger se dispara automáticamente
--   - Inserta un registro en "users_audit" con:
--       -> user_id
--       -> acción 'UPDATE'
--       -> datos actuales del usuario después de la actualización
-- ===========================================
...
BEGIN
    INSERT INTO users_audit (user_id, action, username, email, phone, dni, role_id)
    VALUES (NEW.id, 'UPDATE', NEW.username, NEW.email, NEW.phone, NEW.dni, NEW.role_id);
END$$
DELIMITER ;
